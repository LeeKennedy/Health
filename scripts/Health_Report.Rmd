...
Looking more closely at that challenge and its impact on my weight, we see that, having peaked (or troughed) in early March there is a trend upwards.  After my birthday^[Tomorrow] effort must be put into reversing this trend:
...
weight5 <- cbind(weight4, w2h)

whr <- ggplot(weight5[3000:nrow(weight5),], aes(x = Date, y = w2h)) + 
        geom_point(shape = 21, fill = "cornflowerblue", colour = "blue", size = 4) +
        geom_smooth(colour = "darkgreen") +
